...
it('非空数据库与原件备份可恢复到独立目录，损坏文件被拒绝',async()=>{
 ...
 try{
  ...
  await saveEvent(tx,auth,{requestKey:randomUUID(),occurredOn:'2025-06-01',title:'恢复测试',body:'原始正文',feeling:'',uploadIds:[u.id]});const temporary=await authorizeUpload(tx,auth,{name:'temporary.jpg',size:123});await db.query("update momentnest.media set capture_time_override='2025-07-26T19:27:16'");const folder=join(root,'backup');expect(await exportSnapshot(db,folder,join(root,'media'))).toEqual({records:1,originals:1});
  expect(await finalizeBackup(folder)).toMatchObject({exportedFiles:1,reviewFiles:0});
