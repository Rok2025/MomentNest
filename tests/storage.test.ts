...
describe('real local immutable storage and processing',()=>{
 ...
 it('真实照片生成预览，未知拍摄时间不被上传时间替代，原件哈希不变',async()=>{const data=await sharp({create:{width:2400,height:1600,channels:3,background:'#557744'}}).png().toBuffer(),k=key();await fetch(ticket(k,'put',data.length),{method:'PUT',headers:{Origin:origin},body:data});const sha=createHash('sha256').update(data).digest('hex');const result=await processMedia({mediaId:randomUUID(),token:randomUUID(),generation:1,attempts:1,key:k,kind:'image',sha256:sha});expect((result.metadata.previewSizes as {width:number}[]).map(s=>s.width)).toEqual([480,960,1920]);expect(result.capturedText).toBeNull();expect((await sharp(objectPath(result.previewKey)).metadata()).format).toBe('jpeg');expect(createHash('sha256').update(await readFile(objectPath(k))).digest('hex')).toBe(sha);});
 ...
 it('HEVC/HDR MOV转成H264播放版及封面，保留原件与拍摄时间',async()=>{
  ...
  const job={mediaId:randomUUID(),token:randomUUID(),generation:1,attempts:1,key:k,kind:'video',sha256:sha};
  const early=await processMedia(job,'preview');
  expect(early.playbackKey).toBeNull();
  await expect(readFile(objectPath(early.previewKey.replace(/\.jpg$/,'.mp4')))).rejects.toThrow();
  const posterBytes=await readFile(objectPath(early.previewKey));
  const encoding={...job,token:randomUUID(),previewKey:early.previewKey,metadata:early.metadata};
