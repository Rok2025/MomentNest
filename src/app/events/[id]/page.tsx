...
export default async function Detail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{saved?:string}>}){
 ...
 const saved=(await searchParams).saved==='1';return <><Header label={c.member.label}/><article className="detail panel"><Back memberId={c.member.id}/>{saved&&<p className="success" role="status">记录已保存</p>}<p className="eyebrow">{e.author}留下的回忆</p>{e.title.trim()?<h1>{e.title}</h1>:<h1 className="sr-only">{e.occurredOn}的回忆</h1>}<p>{e.occurredOn} <span className="age">{ageOn(e.occurredOn)}</span></p>{e.body&&<p className="preserve story">{e.body}</p>}{e.feeling&&<blockquote className="preserve">{e.feeling}</blockquote>}<MediaGallery eventId={id} initial={await listMedia(database(),c.authId,id,true)}/><div className="metadata"><p>创建于 {displayTimestamp(e.createdAt)}</p><p>最后编辑：{e.editor} · {displayTimestamp(e.updatedAt)}</p></div><Link className="button primary" href={`/events/${id}/edit`}>补充 / 编辑</Link></article></>;
}
