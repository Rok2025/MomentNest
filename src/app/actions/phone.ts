...
async function bindingAccount(auth:Awaited<ReturnType<typeof authClient>>){
 const {data,error}=await auth.auth.getUser();
 if(error||!data.user)throw Error('UNAUTHENTICATED');
 await memberFor(database(),data.user.id);
 return data.user;
}
export async function sendPhoneCode(input:{phone:string;bind:boolean}):Promise<AuthResult>{
...
export async function verifyPhoneCode(input:{phone:string;code:string;bind:boolean;expectedMemberId?:string}):Promise<AuthResult>{
