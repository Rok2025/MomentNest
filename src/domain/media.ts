...
export type MediaRecord={id:string;eventId:string;filename:string;kind:MediaKind;mime:string;size:number;position:number;status:'pending'|'processing'|'ready'|'failed';uploader:'爸爸'|'妈妈'|null;needsTimeReview?:boolean;capturedText:string|null;capturedZone:string|null;captureTimeOverride?:string|null;errorCode:string|null;hasPreview:boolean;hasPlayback:boolean;preview?:PreviewLinks;playback?:PlaybackLink};
...
export function fileProblem(name:string,size:number):string|null{const kind=fileKind(name);if(!kind)return '支持 JPEG、PNG、WebP、HEIC/HEIF 照片与 MOV/MP4 视频';if(size<=0)return '文件为空';if(size>(kind==='image'?MAX_IMAGE_BYTES:MAX_VIDEO_BYTES))return kind==='image'?'照片不能超过50 MB':'视频不能超过1 GB';return null;}
