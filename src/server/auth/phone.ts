...
function sendFailure(error:{status?:number;code?:string}):AuthResult{
 return error.status===429||error.code?.includes('rate_limit')
  ?{ok:false,message:'验证码请求过于频繁，请稍后再试'}:unavailable;
}
function logUnexpectedFailure(stage:string,error:unknown){
 // Provider messages may include a phone number or OTP, so logs keep only a category.
 console.error(JSON.stringify({event:'phone_auth_unexpected_failure',stage,errorType:error instanceof Error?error.name:'unknown'}));
}
...
export async function sendPhoneCodeWithAuth(input:{phone:string;bind:boolean},createAuth:PhoneAuthFactory):Promise<AuthResult>{
 ...
 }catch(error){logUnexpectedFailure('send',error);return unavailable;}
}
export async function verifyPhoneCodeWithAuth(input:{phone:string;code:string;bind:boolean;expectedMemberId?:string},createAuth:PhoneAuthFactory):Promise<AuthResult>{
 ...
 try{
  ...
  if(error||!data.user)return {ok:false,message:error?.status===429?'验证过于频繁，请稍后重试':'验证码错误或已过期，请重试'};
  try{
   ...
   try{await auth.auth.signOut({scope:'local'});}catch(error){logUnexpectedFailure('sign_out_after_rejected_verification',error);}
   return {ok:false,message:'请使用原家庭成员账号登录'};
  }
  return {ok:true,message:parsed.data.bind?'手机号已绑定，可使用验证码登录':'登录成功'};
 }catch(error){logUnexpectedFailure('verify',error);return unavailable;}
}
