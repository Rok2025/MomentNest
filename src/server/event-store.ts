...
import { initialDayMedia,lockUploads,mediaRecord,type DayMedia } from './media-store';
...
export type TimelinePage = { items: EventRecord[]; next: Cursor | null; dayMedia: Record<string,DayMedia> };
export async function listEvents(db: Queryable, authId: string, cursor?: Cursor, range?: {start:string;end:string},withPreviewLinks=false,filters:TimelineFilters={}) {
 const m = await memberFor(db,authId);
 return listForMember(db,m,cursor,range,withPreviewLinks,filters);
}
async function listForMember(db: Queryable, m: Member, cursor?: Cursor, range?: {start:string;end:string},withPreviewLinks=false,filters:TimelineFilters={}):Promise<TimelinePage> {
  ...
  if(filters.author){const n=values.length;values.push(filters.author);after+=` and a.label=$${n+1}`;}
  if(filters.kind==='text')after+=" and (length(trim(e.title))>0 or length(trim(e.body))>0 or length(trim(e.feeling))>0)";
  if(filters.kind==='image'||filters.kind==='video'){const n=values.length;values.push(filters.kind);after+=` and exists(select 1 from momentnest.media filtered_media where filtered_media.household_id=e.household_id and filtered_media.event_id=e.id and filtered_media.kind=$${n+1})`;}
  ...
  const dayMedia=await initialDayMedia(db,m,items.map(item=>item.occurredOn),withPreviewLinks,items.map(item=>item.id));
  return { items, next: rows.length>20 && last ? { date:last.occurredOn,createdAt:last.createdAt,id:last.id } : null,dayMedia };
}
...
export async function homeData(db:Queryable,authId:string,range?:{start:string;end:string},pages=1,withPreviewLinks=false,filters:TimelineFilters={}){
  const member=await memberFor(db,authId);
  const [first,days]=await Promise.all([listForMember(db,member,undefined,range,withPreviewLinks,filters),countsForMember(db,member)]);
  let page=first;
  for(let i=1;i<Math.min(50,pages)&&page.next;i++){
    const next=await listForMember(db,member,page.next,range,withPreviewLinks,filters);
    page={items:[...page.items,...next.items],next:next.next,dayMedia:{...page.dayMedia,...next.dayMedia}};
  }
