...
import {automaticUploadTime,mobileUploadAgent,type ConfirmedUploadTime} from '@/domain/upload-time';
...
export type UploadItem={key:string;file:{name:string;size:number;type?:string;lastModified?:number};blob?:File;id?:string;sha256?:string;status:'hashing'|'preparing'|'waiting'|'uploading'|'verifying'|'verified'|'failed';progress:number;uploadedBytes?:number;error?:string;occurredOn?:string;capturedOn?:string|null;capturedText?:string|null;dateEdited?:boolean;dateSaving?:boolean;requiresTimeConfirmation?:boolean;confirmedTime?:ConfirmedUploadTime;automaticTime?:boolean;capturedZone?:string|null;existingYoyo?:boolean;reused?:boolean;requestKey?:string};
...
export function Uploader({items,onChange,disabled,onExpired,date,today,memberId}:{items:UploadItem[];onChange:(items:UploadItem[])=>void;disabled:boolean;onExpired:()=>void;date:string;today:string;memberId:string}){
 ...
 async function verify(item:UploadItem){
  ...
  const automatic = latest.requiresTimeConfirmation && !latest.confirmedTime
    ? automaticUploadTime({capturedText:result.capturedText,capturedZone:result.capturedZone,captureReliable:result.captureReliable,yoyotime:result.yoyotime},today)
    : null;
  patch(item.key,{status:'verified',blob:undefined,sha256:result.sha256,progress:100,uploadedBytes:item.file.size,capturedOn:result.capturedOn,capturedText:result.capturedText,capturedZone:result.capturedZone,existingYoyo:!!result.yoyotime?.valid,...(automatic?{confirmedTime:automatic,automaticTime:true,occurredOn:captureDate(automatic.value)||'',dateEdited:true}:!latest.dateEdited&&!latest.confirmedTime?{occurredOn:result.archiveDate||result.capturedOn||'',dateEdited:!!result.archiveDate}:{})});
 }
 ...
 {items.length>0&&<details ref={details} className="upload-details" open={open} onToggle={e=>setOpen(e.currentTarget.open)}><summary><span>文件上传详情 · {items.length} 份</span><span>{open?'收起':'展开'}</span></summary>{open&&<ul className="upload-list">{items.map(item=><li key={item.key}><div className="upload-info"><strong>{item.file.name}</strong><small>{formatUploadBytes(item.file.size)} · {label(item)}</small>{item.reused&&<small>已复用之前的上传任务</small>}{item.status==='verified'&&(item.requiresTimeConfirmation?<UploadTimeConfirmation name={item.file.name} text={item.capturedText||null} zone={item.capturedZone||null} existingYoyo={!!item.existingYoyo} confirmed={item.confirmedTime} automatic={item.automaticTime} today={today} disabled={disabled} onChange={confirmedTime=>patch(item.key,{confirmedTime,automaticTime:false,occurredOn:confirmedTime?captureDate(confirmedTime.value)||'':'',dateEdited:!!confirmedTime})}/>:<div className="media-date">{!item.capturedOn&&<small className="upload-attention">拍摄日期未知，必须确认后保存</small>}<span>{item.dateEdited?'归档日期（已选择）':item.capturedOn?'拍摄日期':'确认归档日期'}</span><DateField label={`${item.file.name}的日期`} min={BIRTHDAY} max={today} required disabled={disabled||item.dateSaving} value={item.occurredOn||''} onChange={value=>void changeDate(item,value)}/>{!item.capturedOn&&!uploadDateReady(item,today)&&<button type="button" disabled={disabled||item.dateSaving} onClick={()=>void changeDate(item,date)}>确认使用记录日期 {date}</button>}</div>)}</div>{(item.status==='failed'||item.status==='verified')&&<button type="button" disabled={disabled} onClick={()=>retry(item)}>{item.status==='failed'&&item.blob?'重试':'重新核验'}</button>}{!item.blob&&item.status==='failed'&&<label className="button">重新选择原文件<input className="sr-only" type="file" accept={accept} disabled={disabled} onChange={e=>select(e.currentTarget,item)}/></label>}<button type="button" disabled={disabled} onClick={()=>void remove(item)}>{['uploading','hashing'].includes(item.status)?'取消':'移除'}</button></li>)}</ul>}</details>}
 </section></>;
}
