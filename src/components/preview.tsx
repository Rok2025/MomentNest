...
export function Preview({today}:{today:string}){
 ...
 return <><p className="notice">本地界面预览 · 以下均为虚构示例，未写入家庭数据库。<button onClick={()=>setEditor(v=>!v)}>{editor?'查看时间线':'查看记录表单'}</button></p>{editor?<EventEditor preview today={today} memberId="00000000-0000-4000-8000-000000000000"/>:<Timeline initial={{items:events,next:null,dayMedia:{}}} days={days} today={today} memberId="preview" demoEvents={events}/>}</>;
}
