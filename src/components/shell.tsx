...
export function Header({label}:{label?:string}){return <header className="topbar"><Link className="brand" href="/">拾光记<span>又又的成长手账</span></Link><nav>{label&&<span>{label}</span>}{label?<Logout/>:<Link href="/login">登录</Link>}</nav></header>;}
