...
每条最多100份文件，照片50 MiB、视频500 MiB。支持JPEG/PNG/WebP/HEIC/HEIF及MOV/MP4/M4V；Live Photo照片与视频分别保存，不做配对动态播放。原件保留收到的字节，未知拍摄时间明确标为未知。

升级时，按时间顺序应用 `supabase/migrations/` 中尚未执行的迁移（包括50份和100份上限迁移），再发布应用，确保数据库允许保存100份素材。发布前检查会在数据库仍使用旧上限时阻止应用切换。
