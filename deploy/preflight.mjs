...
try {
  ...
  for (const [name, worker] of [['DATABASE_URL', false], ['WORKER_DATABASE_URL', true]]) {
    ...
    try {
      await pool.query('select id from momentnest.media_jobs limit 0');
  ...
  console.error('Production preflight failed; credentials and connection details withheld. Check runtime.env, network, permissions and disk.');
  process.exitCode = 1;
}
